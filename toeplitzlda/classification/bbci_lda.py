...
class BenchmarkLda(ShrinkageLinearDiscriminantAnalysis):
    def __init__(
        self,
        priors=None,
        only_block=False,
        ...
        self.use_predict_cov = use_predict_cov
        self.use_global_mean = use_global_mean
        self.normalize_weights = normalize_weights

    def fit(self, X_train, y, xcov=None):
        self.classes_ = sklearn.utils.multiclass.unique_labels(y)
        if set(self.classes_) != {0, 1}:
            raise ValueError("currently only binary class supported")
        assert len(X_train) == len(y)
        xTr_mean = X_train.T
        xTr_cov = X_train.T.copy()
        y_mean = y
        y_cov = y.copy() if not self.use_global_mean else np.ones_like(y)
        n_samples = xTr_mean.shape[1]
        if self.data_use_ratio is not None:
            n_for_mean = int(np.ceil(self.data_use_ratio["mean"] * n_samples))
            n_for_cov = int(np.ceil(self.data_use_ratio["cov"] * n_samples))
            if self.stochastic_sampling:
                mean_idx = np.random.choice(range(n_samples), n_for_mean)
                cov_idx = np.random.choice(range(n_samples), n_for_cov)
            else:
                mean_idx = list(range(n_for_mean))
                cov_idx = list(range(n_for_cov))

            xTr_mean = xTr_mean[:, mean_idx]
            xTr_cov = xTr_cov[:, cov_idx]
            y_mean = y_mean[mean_idx]
            y_cov = y_cov[cov_idx]
        n_classes = 2
        if self.priors is None:
            # here we deviate from the bbci implementation and
            # use the sample priors by default
            _, y_t = np.unique(y_mean, return_inverse=True)  # non-negative ints
            priors = np.bincount(y_t) / float(len(y))
            # self.priors = np.array([1./n_classes] * n_classes)
        else:
            priors = self.priors

        _, cl_mean = subtract_classwise_means(xTr_mean, y_mean)
        X_cov, _ = subtract_classwise_means(xTr_cov, y_cov)
        if self.pool_cov:
            C_cov, C_gamma = shrinkage(
                X_cov,
                n_channels=self.n_channels,
            ...
            n_classes = 2
            C_cov = np.zeros((xTr_cov.shape[0], xTr_cov.shape[0]))
            for cur_class in range(n_classes):
                class_idxs = y_cov == cur_class
                x_slice = X_cov[:, class_idxs]
                C_cov += priors[cur_class] * shrinkage(x_slice)[0]
        ...
        self.coef_ = w.reshape((1, -1))
        self.intercept_ = b
        self.cl_mean_ = cl_mean
        self.priors_ = priors
        if self.use_predict_cov and self.use_global_mean:
            self.X_cov_ = X_cov
    ...
    def decision_function(self, X):
        ...
        if self.use_predict_cov:
            X_cov, _ = subtract_classwise_means(X.T, np.ones((X.shape[0],)))
            if self.use_global_mean:
                X_cov = np.append(X_cov, self.X_cov_, axis=1)
            C_cov, _ = shrinkage(
                X_cov,
                n_channels=self.n_channels,
                n_times=self.N_times,
            ...
            new_w = np.linalg.lstsq(C_cov, self.cl_mean_)[0]
            new_b = -0.5 * np.sum(self.cl_mean_ * new_w, axis=0).T + np.log(self.priors_)
            new_w = new_w[:, 1] - new_w[:, 0]
            new_coef = new_w.reshape((1, -1))
            new_b = new_b[1] - new_b[0]

        dot_coefs = new_coef.T if self.use_predict_cov else self.coef_.T
        dot_intercept = new_b if self.use_predict_cov else self.intercept_
        ...
        scores = safe_sparse_dot(X, dot_coefs, dense_output=True) + dot_intercept
