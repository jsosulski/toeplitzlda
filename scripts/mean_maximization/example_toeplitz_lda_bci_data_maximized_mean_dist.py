...
# %%
""" Choose letter that maximizes mean distance
...
Assumptions:
- The true target/nontarget assignments maximize distance between class means
"""
...
sanity_check_letter_for_data_based_random_means = "G"  # only relevant if previous variable > 0
...
channels = list()
# fmt: off
# channels.append(["Fp1", "Fp2", "F3", "F4", "C3", "C4", "P3", "P4", "O1", "O2", "F7", "F8", "T7",
...
for hyp_i, (use_toeplitz_covariance, use_aggregated_mean) in enumerate(parameter_combinations):
    ...
    for sub in subjects:
        ...
        for block in blocks:
            ...
            all_epochs_1_4 = mne.concatenate_epochs(
                [all_epochs[f"Letter_{i+1}"][:17] for i in range(n_letters)]
            )
            all_epochs_2_4 = mne.concatenate_epochs(
                [all_epochs[f"Letter_{i+1}"][:34] for i in range(n_letters)]
            )
            all_epochs_3_4 = mne.concatenate_epochs(
                [all_epochs[f"Letter_{i+1}"][:51] for i in range(n_letters)]
            )
            all_epochs_quarters = [all_epochs, all_epochs_3_4, all_epochs_2_4, all_epochs_1_4]
            data_quarters = [4, 3, 2, 1]
            # Need to obtain feature dimensions from loaded data
            for chans in channels:
                row["channels"] = "_".join(chans)
                row["n_channels"] = len(chans)
                [aeq.pick_channels(chans) for aeq in all_epochs_quarters]
                ...
                for dqi, dq in enumerate(data_quarters):
                    ...
                    for let_i in range(n_letters):
                        ...
                        epo_current_trial = all_epo[f"Letter_{let_i + 1}"]
                        ...
                        for si, s in enumerate(spellable):
                            try:
                                ...
                                print(
                                    f"Could not select for symbol {s} as it does not exist in epos"
                                )
                                continue
                            ...
                            try:
                                ...
                                num_gt_targets = 0
                            # if debug_prints:
                            #     print(f"Forbidden information: {num_gt_targets} actual targets in subset")
                            ...
                            if aggregated_clmeans[dqi] is None:
                                print(f"Initializing class means to 0.")
                                aggregated_clmeans[dqi] = np.zeros((2, n_times * n_channels))
                            aggregated_trial_means = (
                                aggregated_clmeans[dqi] * let_i + current_trial_means
                            ) / (let_i + 1)
                            if use_aggregated_mean:
                                ...
                                sq_agg_mean_dist = (agg_mean_diff.T @ total_prec @ agg_mean_diff)[
                                    0, 0
                                ]
                        ...
                        aggregated_clmeans[dqi] = (
                            aggregated_clmeans[dqi] * let_i + best_mean_estimate
                        ) / (let_i + 1)
                        ...
                        print(
                            f" Decoded/Actual:  {spellable[most_likely_idx]}/{true_sentence[let_i]}"
                        )
                            ...
                            for chi, ch in enumerate(plot_me_channels):
                                ...
                                for axi, (description, mean) in enumerate(
                                    [
                                        ...
                                        ("aggregated trials", aggregated_clmeans[0]),
                                    ]
                                ):
                                    ch_mean = np.array(
                                        [
